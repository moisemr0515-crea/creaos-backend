...
const previewImport = async (req, res, next) => {
  try {
    ...
    const resultado = await faqImportService.previsualizarImportacion(req.businessId, file);
...
const confirmImport = async (req, res, next) => {
  try {
    const file = requireFile(req);
    const resultado = await faqImportService.confirmarImportacion(req.businessId, actor(req), file);

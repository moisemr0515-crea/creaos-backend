// ─── ROLES DEL SISTEMA ────────────────────────────────────────────────────────
const ROLES = {
  SUPER_ADMIN: 'superadmin',
  OWNER: 'owner',
  ADMIN: 'admin',
  MANAGER: 'manager',
  SALES: 'sales',
  SUPPORT: 'support',
  VIEWER: 'viewer',
};

// ─── PERMISOS POR MÓDULO ──────────────────────────────────────────────────────
const PERMISSIONS = {
  // Usuarios
  USERS_READ: 'users:read',
  USERS_CREATE: 'users:create',
  USERS_UPDATE: 'users:update',
  USERS_DELETE: 'users:delete',

  // Negocios
  BUSINESSES_READ: 'businesses:read',
  BUSINESSES_UPDATE: 'businesses:update',
  BUSINESSES_SETTINGS: 'businesses:settings',

  // Leads
  LEADS_READ: 'leads:read',
  LEADS_CREATE: 'leads:create',
  LEADS_UPDATE: 'leads:update',
  LEADS_DELETE: 'leads:delete',
  LEADS_OWN: 'leads:own', // solo sus propios leads

  // Pipeline
  PIPELINE_READ: 'pipeline:read',
  PIPELINE_UPDATE: 'pipeline:update',

  // Productos (CREA Product Intelligence™)
  PRODUCTS_READ: 'products:read',
  PRODUCTS_CREATE: 'products:create',
  PRODUCTS_UPDATE: 'products:update',
  PRODUCTS_DELETE: 'products:delete',

  // Policies (CREA SALES AI™ C.2 — Business Brain). RBAC separado de
  // products:*/faqs:* — decisión confirmada en la auditoría (sección 6,
  // decisión #1): mismo nivel de granularidad que el resto del RBAC del
  // proyecto, NO un "business-knowledge:*" unificado.
  POLICIES_READ: 'policies:read',
  POLICIES_CREATE: 'policies:create',
  POLICIES_UPDATE: 'policies:update',
  POLICIES_DELETE: 'policies:delete',

  // FAQs (CREA SALES AI™ C.2 — Business Brain)
  FAQS_READ: 'faqs:read',
  FAQS_CREATE: 'faqs:create',
  FAQS_UPDATE: 'faqs:update',
  FAQS_DELETE: 'faqs:delete',

  // Mensajes
  MESSAGES_READ: 'messages:read',
  MESSAGES_CREATE: 'messages:create',

  // Configuración
  SETTINGS_READ: 'settings:read',
  SETTINGS_UPDATE: 'settings:update',

  // Reportes
  REPORTS_READ:   'reports:read',
  REPORTS_EXPORT: 'reports:export',

  // Panel Admin
  ADMIN_READ:  'admin:read',
  ADMIN_WRITE: 'admin:write',

  // Notificaciones
  NOTIFICATIONS_READ: 'notifications:read',

  // Roles
  ROLES_READ: 'roles:read',
  ROLES_MANAGE: 'roles:manage',
};

// Permisos agrupados por rol del sistema
const ROLE_PERMISSIONS = {
  [ROLES.SUPER_ADMIN]: Object.values(PERMISSIONS),

  [ROLES.OWNER]: [
    PERMISSIONS.USERS_READ,
    PERMISSIONS.USERS_CREATE,
    PERMISSIONS.USERS_UPDATE,
    PERMISSIONS.USERS_DELETE,
    PERMISSIONS.BUSINESSES_READ,
    PERMISSIONS.BUSINESSES_UPDATE,
    PERMISSIONS.BUSINESSES_SETTINGS,
    PERMISSIONS.LEADS_READ,
    PERMISSIONS.LEADS_CREATE,
    PERMISSIONS.LEADS_UPDATE,
    PERMISSIONS.LEADS_DELETE,
    PERMISSIONS.PIPELINE_READ,
    PERMISSIONS.PIPELINE_UPDATE,
    PERMISSIONS.PRODUCTS_READ,
    PERMISSIONS.PRODUCTS_CREATE,
    PERMISSIONS.PRODUCTS_UPDATE,
    PERMISSIONS.PRODUCTS_DELETE,
    PERMISSIONS.POLICIES_READ,
    PERMISSIONS.POLICIES_CREATE,
    PERMISSIONS.POLICIES_UPDATE,
    PERMISSIONS.POLICIES_DELETE,
    PERMISSIONS.FAQS_READ,
    PERMISSIONS.FAQS_CREATE,
    PERMISSIONS.FAQS_UPDATE,
    PERMISSIONS.FAQS_DELETE,
    PERMISSIONS.MESSAGES_READ,
    PERMISSIONS.MESSAGES_CREATE,
    PERMISSIONS.SETTINGS_READ,
    PERMISSIONS.SETTINGS_UPDATE,
    PERMISSIONS.REPORTS_READ,
    PERMISSIONS.REPORTS_EXPORT,
    PERMISSIONS.ROLES_READ,
    PERMISSIONS.ROLES_MANAGE,
    PERMISSIONS.ADMIN_READ,
    PERMISSIONS.ADMIN_WRITE,
    PERMISSIONS.NOTIFICATIONS_READ,
  ],

  [ROLES.ADMIN]: [
    PERMISSIONS.USERS_READ,
    PERMISSIONS.USERS_CREATE,
    PERMISSIONS.USERS_UPDATE,
    PERMISSIONS.BUSINESSES_READ,
    PERMISSIONS.BUSINESSES_UPDATE,
    PERMISSIONS.LEADS_READ,
    PERMISSIONS.LEADS_CREATE,
    PERMISSIONS.LEADS_UPDATE,
    PERMISSIONS.LEADS_DELETE,
    PERMISSIONS.PIPELINE_READ,
    PERMISSIONS.PIPELINE_UPDATE,
    PERMISSIONS.PRODUCTS_READ,
    PERMISSIONS.PRODUCTS_CREATE,
    PERMISSIONS.PRODUCTS_UPDATE,
    PERMISSIONS.PRODUCTS_DELETE,
    PERMISSIONS.POLICIES_READ,
    PERMISSIONS.POLICIES_CREATE,
    PERMISSIONS.POLICIES_UPDATE,
    PERMISSIONS.POLICIES_DELETE,
    PERMISSIONS.FAQS_READ,
    PERMISSIONS.FAQS_CREATE,
    PERMISSIONS.FAQS_UPDATE,
    PERMISSIONS.FAQS_DELETE,
    PERMISSIONS.MESSAGES_READ,
    PERMISSIONS.MESSAGES_CREATE,
    PERMISSIONS.SETTINGS_READ,
    PERMISSIONS.SETTINGS_UPDATE,
    PERMISSIONS.REPORTS_READ,
    PERMISSIONS.ROLES_READ,
    PERMISSIONS.ADMIN_READ,
    PERMISSIONS.ADMIN_WRITE,
    PERMISSIONS.NOTIFICATIONS_READ,
  ],

  [ROLES.MANAGER]: [
    PERMISSIONS.USERS_READ,
    PERMISSIONS.LEADS_READ,
    PERMISSIONS.LEADS_CREATE,
    PERMISSIONS.LEADS_UPDATE,
    PERMISSIONS.LEADS_DELETE,
    PERMISSIONS.PIPELINE_READ,
    PERMISSIONS.PIPELINE_UPDATE,
    PERMISSIONS.PRODUCTS_READ,
    PERMISSIONS.PRODUCTS_CREATE,
    PERMISSIONS.PRODUCTS_UPDATE,
    PERMISSIONS.PRODUCTS_DELETE,
    PERMISSIONS.POLICIES_READ,
    PERMISSIONS.POLICIES_CREATE,
    PERMISSIONS.POLICIES_UPDATE,
    PERMISSIONS.POLICIES_DELETE,
    PERMISSIONS.FAQS_READ,
    PERMISSIONS.FAQS_CREATE,
    PERMISSIONS.FAQS_UPDATE,
    PERMISSIONS.FAQS_DELETE,
    PERMISSIONS.MESSAGES_READ,
    PERMISSIONS.MESSAGES_CREATE,
    PERMISSIONS.REPORTS_READ,
    PERMISSIONS.ADMIN_READ,
    PERMISSIONS.NOTIFICATIONS_READ,
  ],

  // SALES/SUPPORT solo lectura de catálogo (mismo criterio que ya aplica
  // PIPELINE_READ-sin-UPDATE a Sales hoy) — gestionar precio/stock/catálogo
  // es una acción de configuración del negocio, no de trabajo diario con un
  // lead puntual; igual que Pipeline, queda reservado a Owner/Admin/Manager.
  // Mismo criterio se extiende a policies:*/faqs:* — configurar las reglas
  // de negocio que usa la IA para responder no es trabajo diario de venta.
  [ROLES.SALES]: [
    PERMISSIONS.LEADS_READ,
    PERMISSIONS.LEADS_CREATE,
    PERMISSIONS.LEADS_UPDATE,
    PERMISSIONS.LEADS_OWN,
    PERMISSIONS.PIPELINE_READ,
    PERMISSIONS.PRODUCTS_READ,
    PERMISSIONS.POLICIES_READ,
    PERMISSIONS.FAQS_READ,
    PERMISSIONS.MESSAGES_READ,
    PERMISSIONS.MESSAGES_CREATE,
    PERMISSIONS.NOTIFICATIONS_READ,
  ],

  [ROLES.SUPPORT]: [
    PERMISSIONS.LEADS_READ,
    PERMISSIONS.MESSAGES_READ,
    PERMISSIONS.MESSAGES_CREATE,
    PERMISSIONS.NOTIFICATIONS_READ,
  ],

  [ROLES.VIEWER]: [
    PERMISSIONS.LEADS_READ,
    PERMISSIONS.PIPELINE_READ,
    PERMISSIONS.PRODUCTS_READ,
    PERMISSIONS.POLICIES_READ,
    PERMISSIONS.FAQS_READ,
    PERMISSIONS.REPORTS_READ,
  ],
};

// Planes de suscripción
const PLANS = {
  TRIAL: 'trial',
  STARTER: 'starter',
  PRO: 'pro',
  ENTERPRISE: 'enterprise',
};

const PLAN_STATUS = {
  ACTIVE: 'active',
  TRIAL: 'trial',
  EXPIRED: 'expired',
  CANCELLED: 'cancelled',
};

// Duración del trial en días
const TRIAL_DAYS = 14;

// Prefijos de Redis
const REDIS_KEYS = {
  REFRESH_TOKEN: (userId, jti) => `rt:${userId}:${jti}`,
  RESET_TOKEN: (token) => `reset:${token}`,
  LOGIN_ATTEMPTS: (ip) => `login_attempts:${ip}`,
  OAUTH_STATE: (state) => `oauth_state:meta:${state}`,
};

module.exports = {
  ROLES,
  PERMISSIONS,
  ROLE_PERMISSIONS,
  PLANS,
  PLAN_STATUS,
  TRIAL_DAYS,
  REDIS_KEYS,
};
